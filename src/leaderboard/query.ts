...
import { LeaderBoardConnection, UserEdge } from "./leaderboard";
import {
...
export default class LeaderBoardQuery {
  ...
  async leaderBoard(
    ...
    if (!after) {
      const contestants = (UserModel.find()
        .sort({ totalQuestionsAnswered: -1, lastAnsweredQuestionTime: 1 })
        .limit(first + 1) as unknown) as User[];
